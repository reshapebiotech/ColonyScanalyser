...
import pytest
from numpy import array, identity, ndarray, zeros
...
@pytest.fixture(params=[(0, 0), (0, -2), (-2, -1), (-0.34, 1.4), (1.5, -2)])
...
class TestDescriptorAlignTransform:
    ...
    def test_init(self, patch_extract_keypoints, transform_type, extractor_type):
        ...
        instance = DescriptorAlignTransform(
            zeros((2, 2)), transform_type, extractor_type
        )
    ...
    def test_image_ref(self, keypoints_count, image_ref, extractor_type):
        instance = DescriptorAlignTransform(
            image_ref, GeometricTransform, extractor_type, n_keypoints=keypoints_count
        )
    ...
    def test_image_align_precise(
        self, mock_align_transform, precise, instance, image_ref, image_translated
    ):
        ...
        if precise:
            ...
            assert (
                instance.align(image_translated, precise=precise).shape
                == image_ref.shape
            )

    @patch("skimage.feature.match_descriptors")
    def test_image_align_transform(
        self, mock_match_descriptors, instance, image_translated
    ):
