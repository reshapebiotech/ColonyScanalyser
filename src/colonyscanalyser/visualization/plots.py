...
def plot_plate_segmented(
    plate_image: ndarray, segmented_image: ndarray, date_time: datetime, save_path: Path
) -> Path:
    """
    ...
    image_path = (
        f"time_point_{date_time.strftime('%Y%m%d')}_{date_time.strftime('%H%M')}.png"
    )
...
def growth_curve(
    ...
    if growth_params:
        ...
        if plate.growth_curve.lag_time.total_seconds() > 0:
            line = ax.axvline(
                plate.growth_curve.lag_time.total_seconds() / 3600,
                color="grey",
                linestyle="dashed",
                alpha=0.5,
            )
        ...
        if plate.growth_curve.carrying_capacity > 0:
            line = ax.axhline(
                plate.growth_curve.carrying_capacity,
                color="blue",
                linestyle="dashed",
                alpha=0.5,
            )
...
def plot_appearance_frequency(  # noqa: C901
    plates: List[Plate], save_path: str, timestamps: List[timedelta] = None, bar=False
) -> Path:
    """
    ...
    for plate in plates:
        ...
        figures.extend(
            time_of_appearance_frequency(
                ax, plate, cm_plate, timestamps=timestamps, bar=bar
            )
        )

    save_params = {"format": "png", "bbox_inches": "tight"}
    if len(plates) > 1:
        lgd = ax.legend(loc="center right", fontsize=8, bbox_to_anchor=(1.25, 0.5))
    ...
    if timestamps is not None:
        plt.xlim(
            ...
            max(timestamps).total_seconds() / 3600,
        )
...
def plot_doubling_map(plates: List[Plate], save_path: Path) -> Path:
    ...
    # Normalise
    weights = zeros_like(x) + 1.0 / len(x)
    ...
    img = plt.imshow(heatmap, cmap="RdPu", aspect="auto", extent=extent, origin="lower")
...
def _image_file_to_plate_images(
    ...
    sliced_images = plate_collection.slice_plate_images(
        image_file.image, background_color=background_color
    )
    ...
    for plate_id, image in sliced_images.items():
        ...
        if image_size_maximum is not None and image.shape > image_size_maximum:
            ...
            image_size = (
                image_size_maximum[0],
                image_size_maximum[0] * (image.shape[1] / image.shape[0]),
            )
            # Resize if required
            image = resize(image, image_size)
